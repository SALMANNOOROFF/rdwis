...
Do Until Len(SourceString) > 220
    SourceString = SourceString & "Hello World"
    Loop

MsgBox "About to test with matching passwords at 1 round"

InterimString = StoreEncryptAES(SourceString, "ABC123", 1)
DestinationString = RetrieveDecryptAES(InterimString, "ABC123", 1)

If SourceString <> DestinationString Then
    MsgBox "Error, Strings do not match"
    Else
    MsgBox "Success, encrypted strings match!", vbInformation
    End If


MsgBox "About to test with matching passwords at 100 rounds"

InterimString = StoreEncryptAES(SourceString, "ABC123", 100)
DestinationString = RetrieveDecryptAES(InterimString, "ABC123", 100)
...
MsgBox "About to test with non-matching passwords at 100 rounds"

InterimString = StoreEncryptAES(SourceString, "ABC123", 100)
DestinationString = RetrieveDecryptAES(InterimString, "AbC123", 100, True)

If SourceString <> DestinationString Then
    MsgBox "Success, Strings do not match", vbInformation
    Else
    MsgBox "Error, encrypted strings match!", vbCritical
    End If

MsgBox "About to throw an error due to non-matching passwords at 100 rounds"
...
Sub ResetPassword(UserName As String)
Dim dbsReset As Database
Dim rstReset As Recordset

Set dbsReset = CurrentDb()
Set rstReset = dbsReset.OpenRecordset("Select acc_pass from cen_accounts where acc_username = '" & UserName & "'", dbOpenDynaset)
If rstReset.EOF Then
    MsgBox "No user with name " & UserName
    Exit Sub
    End If
rstReset.Edit
rstReset!acc_pass = StoreEncryptAES(UserName, "12345", 5)
rstReset.Update

End Sub

Function IsReset(UserName As String, char_seq As String) As Boolean
On Error GoTo Error_Script
If RetrieveDecryptAES(char_seq, "12345", 5) = UserName Then
    IsReset = True
    Else
    IsReset = False
    End If
...
ResetPassword ("rfarwa")
'strEncrypted = StoreEncryptAES("", "12345", 5)
'Debug.Print strEncrypted
'Debug.Print RetrieveDecryptAES(strEncrypted, "12345", 5)
End Sub
