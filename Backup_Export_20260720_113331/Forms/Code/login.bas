...
If StoreEncryptAES(Me.txtUserName, Me.txtPassword, 5) <> rstIdentity!acc_pass Then
    ...
    Exit Sub
    End If
strDesig = rstIdentity!acc_desig
...
Renew_Password:
    ...
    'If this is new SO IT&CYS account, close previous account.
    If strDesig = "Staff Officer IT&CYS" Then
        Set dbs = CurrentDb()
        Set rstPrevAdmin = dbs.OpenRecordset("Select acc_id, acc_status, acc_enddt from cen_accounts Where acc_desig = 'Staff Officer IT&CYS' And acc_status = 'Active'" & _
                                             "And acc_username <> '" & Me.txtUserName & "'")
        rstPrevAdmin.Edit
        rstPrevAdmin!acc_status = "Closed"
        rstPrevAdmin!acc_enddt = GetNow()
        rstPrevAdmin.Update
        MsgBox "Previous Staff Officer IT&CYS account has been closed.", vbInformation
        End If
    End If
