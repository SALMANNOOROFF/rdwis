...
If Not rstAdd.EOF Then
    If Me.acc_desig <> "Staff Officer IT&CYS" Then
        MsgBox "An account for role '" & Me.acc_desig & "' is already open. First close the account to create a new one.", vbCritical
        Exit Sub
        Else
        rstAdd.MoveLast
        If rstAdd.RecordCount > 1 Then
            MsgBox "More than one accounts for role '" & Me.acc_desig & "' are already open. First close one of the accounts to create a new one.", vbCritical
            Exit Sub
            Else
            intresponse = MsgBox("A new account for role '" & Me.acc_desig & "' will be created. On first login of this account, your account will be closed. " & _
                                 "Do you want to proceed?", vbExclamation + vbYesNo, "Confirmation")
