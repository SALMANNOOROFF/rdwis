    ...
    <div class="container-fluid pt-4 px-4 pb-5">
            ...
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h5 class="text-uppercase font-weight-bold rajdhani mb-0" style="letter-spacing:2px; color:var(--rd-text1);">
                         Contract Employees Qualification
                    </h5>
                    <small class="text-muted">Active employees grouped by qualification and department</small>
                </div>
            </div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius:12px; background:var(--rd-surface); overflow:hidden;">
            <div class="table-responsive">
                <table class="table table-sm table-bordered mb-0 qualifications-table" id="qualificationsTable">
                    <thead>
                        <tr>
                            <th class="qual-col">Qulaification</th>
                    ...
                    <tbody>
                        @foreach($qualifications as $qual)
                            @php $rowTotal = $rowTotals[$qual] ?? 0; @endphp
                            <tr class="qual-row">
                                <td class="qual-label">{{ $qual }}</td>
                                <td class="text-center total-cell fw-bold">{{ $rowTotal }}</td>
                                @foreach($headCodes as $hc)
                                    @php $val = $pivot[$qual][$hc] ?? 0; @endphp
                    ...
                    <tfoot>
                        <tr class="total-row">
                            <td class="qual-label fw-bold">Total</td>
        ...
        <div class="row mt-4">
            @foreach($qualifications as $qual)
                @php
                    $total = $rowTotals[$qual] ?? 0;
                    $pct   = $grandTotal > 0 ? round(($total / $grandTotal) * 100) : 0;
                @endphp
                <div class="col-6 col-md-3 col-lg-2 mb-3">
                    <div class="qual-summary-card">
                        <div class="qscard-qual">{{ $qual }}</div>
                        <div class="qscard-count">{{ $total }}</div>
                        <div class="qscard-bar">
                            <div class="qscard-fill" style="width:{{ $pct }}%"></div>
                        </div>
                        <div class="qscard-pct">{{ $pct }}%</div>
                    </div>
                </div>
            @endforeach
        </div>
...
.qual-col  { min-width: 140px;  padding-left: 16px !important; }
.total-col  { min-width: 64px;  background: rgba(23,162,184,0.08) !important; }
.dept-col   { min-width: 58px; }

/* ── Rows ──────────────────────────────────────────────── */
.qual-row { transition: background 0.15s; }
.qual-row:hover td { background: rgba(255,255,255,0.04) !important; }
...
@media print {
    ...
    .qual-summary-card { border: 1px solid #ccc !important; }
    body { background: #fff !important; color: #000 !important; }
    .qualifications-table th, .qualifications-table td { border: 1px solid #999 !important; color: #000 !important; }
...
<script>
function exportCSV() {
    const table = document.getElementById('qualificationsTable');
    ...
    const a    = Object.assign(document.createElement('a'), {
        ...
        download: 'employee_qualifications.csv'
    });
    a.click();
}
</script>
