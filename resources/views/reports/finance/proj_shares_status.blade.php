    ...
    <div class="container-fluid pt-4 px-4 pb-5">
            ...
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h5 class="text-uppercase font-weight-bold rajdhani mb-0" style="letter-spacing:2px; color:var(--rd-text1);">
                        <i class="fas fa-chart-line mr-2 text-warning"></i> Project Shares Status - Comm
                    </h5>
                    <small class="text-muted">Project share breakdown by head</small>
                </div>
            </div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius:12px; background:var(--rd-surface); overflow:hidden;">
            <div class="table-responsive">
                <table class="table table-sm table-bordered table-hover mb-0" id="projSharesTable">
                    <thead style="background-color: var(--rd-surface-lighter); color: var(--rd-text1);">
                        <tr>
                            <th style="min-width:120px;">Head</th>
                            <th class="text-right" style="min-width:120px;">Project Share</th>
                            ...
                            <th class="text-right" style="min-width:120px;">Completed Milestones</th>
                            <th class="text-right" style="min-width:130px;">Available After Milestones</th>
                        ...
                        <tr class="font-weight-bold" style="background-color: rgba(255,255,255,0.07);">
                            <td>Total</td>
                            <td class="text-right">{{ number_format($total['project_share']) }}</td>
                            ...
                            <td class="text-right">{{ number_format($total['completed_milestones']) }}</td>
                            <td class="text-right">{{ number_format($total['available_after_milestones']) }}</td>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $row->head }}</td>
                            <td class="text-right">{{ number_format($row->project_share) }}</td>
                            ...
                            <td class="text-right">{{ number_format($row->completed_milestones) }}</td>
                            <td class="text-right @if($row->available_after_milestones < 0) text-danger @endif">
                                {{ number_format($row->available_after_milestones) }}
                            </td>
...
<style>
    #projSharesTable th, #projSharesTable td { vertical-align: middle; font-size: 0.83rem; }
</style>
@endsection
