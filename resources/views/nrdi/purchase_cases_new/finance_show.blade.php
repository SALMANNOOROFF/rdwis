                    ...
                    <div class="p-3">
                        @forelse($purchase->attachments as $file)
                            <div class="d-flex align-items-center mb-2 p-2 rounded" style="background: var(--rd-neutral-50); border: 1px solid var(--rd-border);">
                                ...
                                <a href="{{ \App\Facades\FileStorage::url($file->pat_path) }}" onclick="window.openLiveDocument('{{ \App\Facades\FileStorage::url($file->pat_path) }}', '{{ addslashes($file->pat_filename ?: 'Supporting Document') }}'); return false;" class="rd-live-file-view btn btn-xs btn-outline-success ml-2" title="View Document Live"><i class="fas fa-eye"></i></a>
                            </div>
                        @empty
