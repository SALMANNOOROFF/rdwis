    ...
    <div class="content-header">
        <div class="container-fluid d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center" style="gap: 8px;">
                ...
                @if(isset($isHr) && $isHr)
                <form action="{{ route('nrdi.contract_cases_new.index') }}" method="GET" class="d-flex align-items-center mr-2 mb-0">
                    <input type="hidden" name="status" value="{{ $status ?? 'open' }}">
                    <select name="division_id" class="form-control form-control-sm" style="min-width: 200px;" onchange="this.form.submit()">
                        <option value="">All Divisions</option>
                        @foreach($divisions as $div)
                            <option value="{{ $div->unt_id }}" {{ ($divisionFilter ?? '') == $div->unt_id ? 'selected' : '' }}>
                                {{ $div->unt_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="btn-group">
                    <a href="{{ route('nrdi.contract_cases_new.index', ['status' => 'open', 'division_id' => $divisionFilter ?? '']) }}"
                       class="btn btn-sm {{ ($status ?? 'open') === 'open' ? 'btn-primary' : 'btn-outline-primary' }}">
                        Open ({{ $openCount ?? 0 }})
                    </a>
                    <a href="{{ route('nrdi.contract_cases_new.index', ['status' => 'closed', 'division_id' => $divisionFilter ?? '']) }}"
