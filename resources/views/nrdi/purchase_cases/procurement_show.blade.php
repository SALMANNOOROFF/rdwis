                        ...
                        <div class="p-3">
                            @forelse($purchase->attachments as $file)
                                <div class="d-flex align-items-center mb-2 p-3 rounded" style="background: var(--rd-neutral-50); border: 1px solid var(--rd-border);">
                                    ...
                                    <a href="{{ \App\Facades\FileStorage::url($file->pat_path) }}" onclick="window.openLiveDocument('{{ \App\Facades\FileStorage::url($file->pat_path) }}', '{{ addslashes($file->pat_filename ?: 'Attachment') }}'); return false;" class="rd-live-file-view btn btn-outline-primary btn-xs shadow-inner">VIEW</a>
                                </div>
                            @empty
