    ...
    <form id="authorityActionForm" action="{{ $isInitiator ? route('purchase.release', $purchase->pcs_id) : route('nrdi.purchase_cases_new.action', $purchase->pcs_id) }}" method="POST">
        ...
        <div class="mb-3">
            <div class="d-flex justify-content-between align-items-center mb-1">
                <span class="text-muted small rajdhani"><i class="fas fa-pen-nib mr-1"></i> Remarks Box <span class="text-danger">*</span></span>
            </div>
            ...
            <div class="mt-2 d-flex flex-wrap gap-2">
                <span class="badge badge-secondary p-1 px-2 cursor-pointer quick-comment-btn" style="font-size: 10px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);">FNA, please.</span>
                <span class="badge badge-secondary p-1 px-2 cursor-pointer quick-comment-btn" style="font-size: 10px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);">Recommended and forwarded.</span>
                <span class="badge badge-secondary p-1 px-2 cursor-pointer quick-comment-btn" style="font-size: 10px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);">For approval please.</span>
                <span class="badge badge-secondary p-1 px-2 cursor-pointer quick-comment-btn" style="font-size: 10px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);">Discussed.</span>
                @if($userArea === 'proc')
                <span class="badge badge-info p-1 px-2 cursor-pointer quick-comment-btn" style="font-size: 10px; background: rgba(23,162,184,0.15); border: 1px solid rgba(23,162,184,0.3); color: #17a2b8;">Quotes verified and recommended.</span>
                @endif
            </div>
        </div>
        ...
        <div class="d-flex" style="gap: 10px; width: 100%;">
            ...
            @else
                ...
                @else
                    @if($isDProcDraft)
                        <button type="button" class="btn btn-outline-warning btn-sm font-weight-bold px-3 mr-2" data-toggle="modal" data-target="#pcAddQuoteModal" style="border-radius: 8px;">
                            <i class="fas fa-plus-circle mr-1"></i> MANAGE QUOTES
                        </button>
                        <button type="button" onclick="handleAction('dproc_save')" id="btnForward" class="dg-btn-action dg-btn-info flex-grow-1" disabled>
...
<script>
    ...
    document.querySelectorAll('.quick-comment-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // If empty or doesn't have prefix, reset
            if (currentVal.trim() === '' || !currentVal.startsWith(prefix)) {
                inlineRemarks.value = prefix + comment;
            } else {
                // Determine if we need to append to the current line
                // Find the last line
                const lines = currentVal.split('\n');
                const lastLine = lines[lines.length - 1];
                
                // If last line just has a number e.g. "2. "
    ...
    window.handleAction = function(action, targetStatus = null) {
        ...
        if ((action === 'return' || action === 'forward' || action === 'dproc_save') && cleanedLines.length === 0) {
            Swal.fire({ title: 'Remarks Required!', text: 'Remarks are compulsory to proceed.', icon: 'warning', background: '#001226', color: '#fff' });
            return;
        }

        if (cleanedLines.length === 0) {
            if (action === 'approve') cleanedLines = ['Case approved'];
            else if (action === 'release') cleanedLines = ['Forwarded for review'];
        }
