...
def kill_existing_servers():
    """Kill any leftover caddy/php processes from previous runs with user choice."""
    choice = input("[??] Do you want to kill/restart all existing Caddy and PHP processes? (y/n) [default: y]: ").strip().lower()
    if choice in ["", "y", "yes"]:
        ...
        print("[OK] Keeping existing processes running (make sure there are no port conflicts!)")
...
def add_domain_to_hosts(ip):
    ...
    try:
        ...
        new_lines.append(f"\n# RDWIS 2.0 - Auto-generated by start_server_2.py")
...
def get_user_inputs():
    """Prompt the user for custom domain and ports before starting with clear descriptions."""
    global LOCAL_DOMAIN, FASTCGI_PORT, HTTP_PORT, HTTPS_PORT
    
    print("=" * 65)
    print("   CONFIGURE PORTS AND DOMAIN FOR THIS PROJECT RUN")
    print("=" * 65)
    print("   Note: Press ENTER directly to keep the default value.")
    print("-" * 65)
    
    # Local Domain
    print("[1] LOCAL DOMAIN NAME")
    print("    This is the address you will type in your browser (e.g. https://rdwis)")
    domain = input(f"    Enter Local Domain Name [default: {LOCAL_DOMAIN}]: ").strip()
    if domain:
        LOCAL_DOMAIN = domain
    print()
        
    # FastCGI Port
    print("[2] PHP FASTCGI PORT")
    print("    This is the internal port where PHP backend workers will run (php-cgi)")
    while True:
        fcgi_port_in = input(f"    Enter FastCGI Port [default: {FASTCGI_PORT}]: ").strip()
        if not fcgi_port_in:
            break
        try:
            FASTCGI_PORT = int(fcgi_port_in)
            break
        except ValueError:
    ...
    # HTTP Port
    print("[3] HTTP PORT (Web Redirect)")
    print("    Standard web port. Any request to this port redirects to HTTPS automatically.")
    while True:
        http_port_in = input(f"    Enter HTTP Port [default: {HTTP_PORT}]: ").strip()
        if not http_port_in:
            break
        try:
            HTTP_PORT = int(http_port_in)
            break
        except ValueError:
            print("    [!] Please enter a valid port number.")
    print()
    ...
    # HTTPS Port
    print("[4] HTTPS PORT (Secure Web Server)")
    print("    The secure port where Caddy serves your website using SSL/HTTPS.")
    while True:
        https_port_in = input(f"    Enter HTTPS Port [default: {HTTPS_PORT}]: ").strip()
        if not https_port_in:
            break
        try:
            HTTPS_PORT = int(https_port_in)
            break
        except ValueError:
            print("    [!] Please enter a valid port number.")
    print()
    ...
    print(f"     Local URL:    https://{LOCAL_DOMAIN}:{HTTPS_PORT}")
    print(f"     PHP FastCGI:  127.0.0.1:{FASTCGI_PORT}")
...
def main():
    print()
    print("=" * 55)
    print("   RDWIS 2.0 - Production Server Launcher (v2)")
    print("=" * 55)
    print()
